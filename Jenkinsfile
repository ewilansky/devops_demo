node {
    ...
    docker.image('gradle:latest').inside('--network=toolchain_demo_tc-net') {
        ...
        stage('Retrieve App') {
            sh 'curl -u admin:admin123 -X GET "http://package-repo:8081/repository/maven-snapshots/org/ahl/springbootdemo/spring-boot-demo/0.0.1-SNAPSHOT/spring-boot-demo-0.0.1-20181102.132114-1.jar" --output ./app.jar'
        }
    }
